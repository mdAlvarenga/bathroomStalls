...
distancesToLeft ((True, n):stalls) tiempoATrue = 0 : distancesToLeft stalls 0 
distancesToLeft ((False, n):stalls) tiempoATrue = tiempoATrue : distancesToLeft stalls (tiempoATrue + 1)
...
maxDistances [] = []
maxDistances ((stall, distance):distances) = (maxDistance stall) : maxDistances distances
...
maximalsOf ((stall, num) : distances) 
	| num > head(maximalsOf distances) = [(stall, num)]
	| num == head(maximalsOf distances) = (stall, num) : (maximalsOf distances)
...
processBathroom (stallQuantity, 0) = updateStalls(processForPerson(buildStalls(stallQuantity))) -- ???????? //ivan puto
processBathroom (stallQuantity, people) =  
		let stalls = buildStalls(stallQuantity) in 
